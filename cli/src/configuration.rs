...
impl GeyserConfig {
    ...
    fn read_extra_conf(&mut self) -> &mut Self {
        if let Some(extras_location) = self.extras_location.as_ref() {
            ...
            parsed
                .into_iter()
                .for_each(|account| self.accounts.push(account));
        }
    ...
    pub fn init(self) -> Self {
        if let Some(error) = std::fs::DirBuilder::new().create(&self.dir).err() {
            if error.kind() == std::io::ErrorKind::AlreadyExists {
                log::info!("APP DIR ALREADY INITIALIZED");
            } else {
    ...
    pub fn start_validator(self) {
        ...
        let config_print_fn = |(message, value): (&str, &str)| {
            println!("  -- {}: {}", message.green(), value.yellow());
        };

        config_print_fn(("Socket", &self.network.to_url()));
        config_print_fn(("App Dir", &self.dir.to_string_lossy()));

        if let Some(user_config_exists) = self.extras_location.as_ref() {
            config_print_fn(("Config", &user_config_exists.to_string_lossy()));
        }
        config_print_fn(("Generated", &self.autogenerated().to_string_lossy()));
        config_print_fn(("Plugin", &self.plugin.to_string_lossy()));

        if self.accounts.is_empty() {
            config_print_fn(("Accounts", "All"));
        } else {
            config_print_fn(("Accounts ", ""));
            for account in self.accounts.as_slice() {
        ...
        status_pb.finish_with_message("Termination Application...");
